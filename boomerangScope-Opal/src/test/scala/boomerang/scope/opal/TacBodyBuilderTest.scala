...
class TacBodyBuilderTest {
  ...
  def isOnIgnoreList(method: Method): Boolean = {
    ...
    if (method.toJava.equals("java.lang.Thread{ private static long nextThreadID() }")) return true
    if (method.toJava.equals("java.util.concurrent.CompletableFuture$Signaller{ public boolean isReleasable() }")) return true
